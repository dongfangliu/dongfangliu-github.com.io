...
function  get_static_Dates(hourlist ,minlist,curr_date) {
    ...
    for(var i = 0 ; i <hourlist.length ;i++){="" dates[i]="new" array(hourlist[i].length);="" for(var="" j="0" ;="" j<hourlist[i].length;j++){="" dates[i][j]="new" date(curr_date.getutcfullyear(),curr_date.getutcmonth(),curr_date.getdate(),hourlist[i][j],minlist[i][j],0);="" }="" return="" dates;="" function="" get_diff_time(dates,curr_date)="" {="" 以下需要持续刷新并部署到网页上="" var="" differ_time_put="new" array(dates.length);="" a="0" <dates.length="" ;a++){="" b="0" b<dates[a].length;b++){="" data="Dates[a][b].getTime()" -="" curr_date.gettime();="" if(data=""> 0){  //此刻尚有班车可等
